    ...
    if($ni > 0){
        for($i=0;$i<$ni;$i++){
            ...
            if($rs_event[$i]['iCampaignBy'] == 1) { // Premise
                $sql = "SELECT e.\"iCampaignCoverageId\", s.\"vName\", st.\"vTypeName\" from event_campaign_coverage e, site_mas s LEFT JOIN site_type_mas st ON s.\"iSTypeId\" = st.\"iSTypeId\" where e.\"iCampaignBy\" = 1 and e.\"iCampaignCoverageId\" = s.\"iSiteId\" and e.\"iEventId\" = '".$rs_event[$i]['iEventId']."' Order BY s.\"iSiteId\"";
