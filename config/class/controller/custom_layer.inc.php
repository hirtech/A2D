...
class CustomLayer {
	...
	function recordset_list()
	{
		...
		$sql = "SELECT custom_layer.* ".$this->join_field_str." FROM custom_layer".$this->join_clause.$this->where_clause.$this->group_by_clause.$this->order_by_clause.$this->limit_clause;
	...
	function recordset_total()
	{
		...
		$sql = "SELECT custom_layer.* ".$this->join_field_str." FROM custom_layer".$this->join_clause.$this->where_clause.$this->group_by_clause;
		//$rs_db = $sqlObj->GetAll($sql);
		//return count($rs_db);
	...
	function delete_records($id){
		...
		$sql_del = "DELETE FROM custom_layer WHERE custom_layer.\"iCLId\" IN (".$id.")";
	...
	function add_records(){
		...
		if($this->insert_arr){
			...
			$sql = "INSERT INTO custom_layer(\"vName\", \"vFile\", \"iStatus\", \"dAddedDate\") VALUES (".gen_allow_null_char($this->insert_arr['vName']).", ".gen_allow_null_char($this->insert_arr['vFile']).", ".gen_allow_null_int($this->insert_arr['iStatus']).",".gen_allow_null_char(date_getSystemDateTime()).")";
			//echo $sql;exit;
			$sqlObj->Execute($sql);		
			$iCLId = $sqlObj->Insert_ID();
	...
	function update_records(){
		...
		if($this->update_arr){
			...
			WHERE \"iCLId\" = ".$this->update_arr['iCLId'];
			//echo $rs_db;exit;
