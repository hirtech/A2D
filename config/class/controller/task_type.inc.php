...
class TaskType {
	...
	function recordset_list()
	{
		...
		$sql = "SELECT task_type_mas.* ".$this->join_field_str." FROM task_type_mas".$this->join_clause.$this->where_clause.$this->group_by_clause.$this->order_by_clause.$this->limit_clause;
	...
	function recordset_total()
	{
		...
		$sql = "SELECT task_type_mas.* ".$this->join_field_str." FROM task_type_mas".$this->join_clause.$this->where_clause.$this->group_by_clause;
		//$rs_db = $sqlObj->GetAll($sql);
		//return count($rs_db);
	...
	function delete_records(){
		...
		$sql = "DELETE task_type_mas ".$this->join_field_str." FROM task_type_mas".$this->join_clause.$this->where_clause.$this->group_by_clause.$this->order_by_clause.$this->limit_clause;
		$rs_db = $sqlObj->Execute($sql);
	...
	function add_records(){
		global $sqlObj;
		if($this->insert_arr){
			$sql = "INSERT INTO task_type_mas (\"vTypeName\", \"iStatus\")VALUES (".gen_allow_null_char($this->insert_arr['vTypeName']).", ".gen_allow_null_char($this->insert_arr['iStatus']).")";
	...
	function update_records(){
		...
		if($this->update_arr){
			$rs_db = "UPDATE task_type_mas SET \"vTypeName\"=".gen_allow_null_char($this->update_arr['vTypeName']).", \"iStatus\"=".gen_allow_null_char($this->update_arr['iStatus'])." WHERE \"iTaskTypeId\" = ".$this->update_arr['iTaskTypeId'];
