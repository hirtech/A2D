...
class EquipmentManufacturer {
	...
	function recordset_list()
	{
		...
		$sql = "SELECT equipment_manufacturer_mas.* ".$this->join_field_str." FROM equipment_manufacturer_mas".$this->join_clause.$this->where_clause.$this->group_by_clause.$this->order_by_clause.$this->limit_clause;
	...
	function recordset_total()
	{
		...
		$sql = "SELECT equipment_manufacturer_mas.* ".$this->join_field_str." FROM equipment_manufacturer_mas".$this->join_clause.$this->where_clause.$this->group_by_clause;
		//$rs_db = $sqlObj->GetAll($sql);
		//return count($rs_db);
	...
	function delete_records($id){
		...
		$sql_del = "DELETE FROM equipment_manufacturer_mas WHERE equipment_manufacturer_mas.\"iEquipmentManufacturerId\" IN (".$id.")";
		$rs_del = $sqlObj->Execute($sql_del);

		/*-------------- Log Entry -------------*/
		$this->SALObj->type = 2;
		$this->SALObj->module_name = "Equipment Manufacturer";
	...
	function add_records(){
		global $sqlObj;
		if($this->insert_arr){
			$sql = "INSERT INTO equipment_manufacturer_mas( \"vEquipmentManufacturer\", \"iStatus\", \"dAddedDate\")VALUES (".gen_allow_null_char($this->insert_arr['vEquipmentManufacturer']).", ".$this->insert_arr['iStatus'].", ".gen_allow_null_char(date_getSystemDateTime()).")";
	...
	function update_records(){
		...
		if($this->update_arr){
			$rs_db = "UPDATE equipment_manufacturer_mas SET  \"vEquipmentManufacturer\"=".gen_allow_null_char($this->update_arr['vEquipmentManufacturer']).", \"iStatus\"=".gen_allow_null_char($this->update_arr['iStatus']).", \"dModifiedDate\" = ".gen_allow_null_char(date_getSystemDateTime())." WHERE \"iEquipmentManufacturerId\" = ".$this->update_arr['iEquipmentManufacturerId'];
