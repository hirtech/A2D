...
class CircuitType {
	...
	function recordset_list()
	{
		...
		$sql = "SELECT circuit_type_mas.* ".$this->join_field_str." FROM circuit_type_mas".$this->join_clause.$this->where_clause.$this->group_by_clause.$this->order_by_clause.$this->limit_clause;
	...
	function recordset_total()
	{
		...
		$sql = "SELECT circuit_type_mas.* ".$this->join_field_str." FROM circuit_type_mas".$this->join_clause.$this->where_clause.$this->group_by_clause;
		//$rs_db = $sqlObj->GetAll($sql);
		//return count($rs_db);
	...
	function delete_records($id){
		...
		$sql_del = "DELETE FROM circuit_type_mas WHERE circuit_type_mas.\"iCircuitTypeId\" IN (".$id.")";
		$rs_del = $sqlObj->Execute($sql_del);

		/*-------------- Log Entry -------------*/
		$this->SALObj->type = 2;
		$this->SALObj->module_name = "Circuit Type";
	...
	function add_records(){
		global $sqlObj;
		if($this->insert_arr){
			$sql = "INSERT INTO circuit_type_mas( \"vCircuitType\", \"iStatus\", \"dAddedDate\")VALUES (".gen_allow_null_char($this->insert_arr['vCircuitType']).", ".$this->insert_arr['iStatus'].", ".gen_allow_null_char(date_getSystemDateTime()).")";
	...
	function update_records(){
		...
		if($this->update_arr){
			$rs_db = "UPDATE circuit_type_mas SET  \"vCircuitType\"=".gen_allow_null_char($this->update_arr['vCircuitType']).", \"iStatus\"=".gen_allow_null_char($this->update_arr['iStatus']).", \"dModifiedDate\" = ".gen_allow_null_char(date_getSystemDateTime())." WHERE \"iCircuitTypeId\" = ".$this->update_arr['iCircuitTypeId'];
