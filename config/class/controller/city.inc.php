...
class City {
	...
	function recordset_list()
	{
		...
		$sql = "SELECT city_mas.* ".$this->join_field_str." FROM city_mas".$this->join_clause.$this->where_clause.$this->group_by_clause.$this->order_by_clause.$this->limit_clause;
	...
	function recordset_total()
	{
		...
		$sql = "SELECT city_mas.* ".$this->join_field_str." FROM city_mas".$this->join_clause.$this->where_clause.$this->group_by_clause;
		//$rs_db = $sqlObj->GetAll($sql);
		//return count($rs_db);
	...
	/*function delete_records(){
		...
		$sql = "DELETE city_mas ".$this->join_field_str." FROM city_mas".$this->join_clause.$this->where_clause.$this->group_by_clause.$this->order_by_clause.$this->limit_clause;
		$rs_db = $sqlObj->Execute($sql);
		
		$this->SALObj->type = 2;
		$this->SALObj->module_name = "city";
	...
	function delete_records($id){
		...
		$sql_del = "DELETE FROM city_mas WHERE city_mas.\"iCityId\" IN (".$id.")";
	...
	function add_records(){
		global $sqlObj;
		if($this->insert_arr){
			$sql = "INSERT INTO city_mas( \"vCity\", \"dAddedDate\")VALUES (".gen_allow_null_char($this->insert_arr['vCity']).",".gen_allow_null_char(date_getSystemDateTime()).")";
	...
	function update_records(){
		...
		if($this->update_arr){
			$rs_db = "UPDATE city_mas SET  \"vCity\"=".gen_allow_null_char($this->update_arr['vCity']).",\"dModifiedDate\" = ".gen_allow_null_char(date_getSystemDateTime())." WHERE \"iCityId\" = ".$this->update_arr['iCityId'];
	...
	function action_records(){
		...
		if($this->ids){
			if($this->action=="Active"){
				$sql = "UPDATE city_mas set \"iStatus\" = '1',\"dModifiedDate\" = ".gen_allow_null_char(date_getSystemDateTime())."  WHERE \"iCityId\" IN (".$this->ids.")";
			}
			if($this->action=="Inactive"){
				$sql = "UPDATE city_mas set \"iStatus\" = '0',\"dModifiedDate\" = ".gen_allow_null_char(date_getSystemDateTime())."  WHERE \"iCityId\" IN (".$this->ids.")";
			}
			$sqlObj->Execute($sql);
