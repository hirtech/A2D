...
class Company {
	...
	function recordset_list()
	{
		...
		$sql = "SELECT company_mas.* ".$this->join_field_str." FROM company_mas".$this->join_clause.$this->where_clause.$this->group_by_clause.$this->order_by_clause.$this->limit_clause;
	...
	function recordset_total()
	{
		...
		$sql = "SELECT company_mas.* ".$this->join_field_str." FROM company_mas".$this->join_clause.$this->where_clause.$this->group_by_clause;
		//$rs_db = $sqlObj->GetAll($sql);
		//return count($rs_db);
	...
	function delete_records($id){
		...
		$sql_del = "DELETE FROM company_mas WHERE company_mas.\"iCompanyId\" IN (".$id.")";
		$rs_del = $sqlObj->Execute($sql_del);

		/*-------------- Log Entry -------------*/
		$this->SALObj->type = 2;
		$this->SALObj->module_name = "Company";
	...
	function add_records(){
		global $sqlObj;
		if($this->insert_arr){
			$sql = "INSERT INTO company_mas(\"vCompanyType\", \"vCompanyName\", \"vNameId\", \"vAccessType\", \"vMSOYr\", \"vMSANum\", \"iStatus\", \"dAddedDate\")VALUES (".gen_allow_null_char($this->insert_arr['vCompanyType']).", ".gen_allow_null_char($this->insert_arr['vCompanyName']).", ".gen_allow_null_char($this->insert_arr['vNameId']).", ".gen_allow_null_char($this->insert_arr['vAccessType']).", ".gen_allow_null_char($this->insert_arr['vMSOYr']).", ".gen_allow_null_char($this->insert_arr['vMSANum']).", ".$this->insert_arr['iStatus'].", ".gen_allow_null_char(date_getSystemDateTime()).")";
			$sqlObj->Execute($sql);		
	...
	function update_records(){
		...
		if($this->update_arr){
			$rs_db = "UPDATE company_mas SET  
				\"vCompanyType\" = ".gen_allow_null_char($this->update_arr['vCompanyType']).",
				\"vCompanyName\" = ".gen_allow_null_char($this->update_arr['vCompanyName']).",
				\"vNameId\" = ".gen_allow_null_char($this->update_arr['vNameId']).",
				\"vAccessType\" = ".gen_allow_null_char($this->update_arr['vAccessType']).",
				\"vMSOYr\" = ".gen_allow_null_char($this->update_arr['vMSOYr']).",
				\"vMSANum\" = ".gen_allow_null_char($this->update_arr['vMSANum']).",
				\"iStatus\" = ".gen_allow_null_char($this->update_arr['iStatus']).",
				\"dModifiedDate\" = ".gen_allow_null_char(date_getSystemDateTime())." 
				WHERE \"iCompanyId\" = ".$this->update_arr['iCompanyId'];
