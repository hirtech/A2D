	...
	function action_records(){		
		...
		if($this->ids){								
			if($this->action=="Active"){
				$sql = "UPDATE site_sub_type_mas set \"iStatus\" = '1',\"dModifiedDate\" = ".gen_allow_null_char(date_getSystemDateTime())."  WHERE \"iSSTypeId\" IN (".$this->ids.")";
			}
			if($this->action=="Inactive"){
				$sql = "UPDATE site_sub_type_mas set \"iStatus\" = '0',\"dModifiedDate\" = ".gen_allow_null_char(date_getSystemDateTime())."  WHERE \"iSSTypeId\" IN (".$this->ids.")";
			}
			$sqlObj->Execute($sql);
