...
class Engagement {
	...
	function recordset_list()
	{
		...
		$sql = "SELECT engagement_mas.* ".$this->join_field_str." FROM engagement_mas".$this->join_clause.$this->where_clause.$this->group_by_clause.$this->order_by_clause.$this->limit_clause;
	...
	function recordset_total()
	{
		...
		$sql = "SELECT engagement_mas.* ".$this->join_field_str." FROM engagement_mas".$this->join_clause.$this->where_clause.$this->group_by_clause;
		//$rs_db = $sqlObj->GetAll($sql);
		//return count($rs_db);
	...
	function delete_records($id){
		...
		$sql_del = "DELETE FROM engagement_mas WHERE engagement_mas.\"iEngagementId\" IN (".$id.")";
		$rs_del = $sqlObj->Execute($sql_del);

		/*-------------- Log Entry -------------*/
		$this->SALObj->type = 2;
		$this->SALObj->module_name = "engagement";
	...
	function add_records(){
		global $sqlObj;
		if($this->insert_arr){
			$sql = "INSERT INTO engagement_mas( \"vEngagement\", \"iStatus\", \"dAddedDate\")VALUES (".gen_allow_null_char($this->insert_arr['vEngagement']).", ".$this->insert_arr['iStatus'].", ".gen_allow_null_char(date_getSystemDateTime()).")";
	...
	function update_records(){
		...
		if($this->update_arr){
			$rs_db = "UPDATE engagement_mas SET  \"vEngagement\"=".gen_allow_null_char($this->update_arr['vEngagement']).", \"iStatus\"=".gen_allow_null_char($this->update_arr['iStatus']).", \"dModifiedDate\" = ".gen_allow_null_char(date_getSystemDateTime())." WHERE \"iEngagementId\" = ".$this->update_arr['iEngagementId'];
