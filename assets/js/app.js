...
/////////////////////////// Wizard Form ////////////////////////
    ...
    var settinghtml = `<div id="settings" class="">
            ...
            <form method="post" name="frm_customize_panel" id="frm_customize_panel" action="">
            <div class="sidbarchat p-3">
                <h5 class="mb-0">PANEL CUSTOMIZER</h5>
                ...
                <ul class="list-inline float-left claerfix">
                    ...
                    <li class="float-left list-inline-item"><input type="color" name="color" class="cursor-pointer color m-2"  value="#3e8ef7"></li>
                </ul>
                <hr class="float-left w-100"/>
                <h6>TEMPLATE LAYOUT</h6>
                <label class="chkbox">Vertical 
                    <input name="layout" value="vertical" class="layout" checked="checked" type="radio" id="layout_vertical">
                    <span class="checkmark"></span>
                </label>
                <label class="chkbox ml-2">Horizontal 
                    <input name="layout" value="horizontal" class="layout" type="radio" id="layout_horizontal">
                    <span class="checkmark"></span>
                </label>
                <hr class="float-left w-100"/>
                <h6>TEMPLATE STYLE</h6>
                <label class="chkbox">Light 
                    <input name="style" value="light" class="style" checked="checked" type="radio" id="style_light" >
                    <span class="checkmark"></span>
                </label> <br/>              
                <label class="chkbox">Semi Dark 
                    <input name="style" value="semi-dark" class="style" type="radio"  id="style_semi-dark">
                    <span class="checkmark"></span>
                </label>
                <label class="chkbox ml-2">Semi Dark Alt 
                    <input name="style" value="semi-dark-alt" class="style" type="radio"  id="style_semi-dark-alt">
                    <span class="checkmark"></span>
                </label><br/>
                <label class="chkbox">Dark 
                    <input name="style" value="dark" class="style" type="radio"  id="style_dark">
                    <span class="checkmark"></span>
                </label>
                <label class="chkbox ml-2">Dark Alt
                    <input name="style" value="dark-alt" class="style" type="radio"  id="style_dark-alt">
                    <span class="checkmark"></span>
                </label>
                <hr class="float-left w-100"/>
                ...
                <hr class="float-left w-100"/>
                <h6></h6>
                <input type="button" name="save_customization" id="btn_custom_panel" value="Save Changes" class="btn btn-primary btn-lg">
            </div>
            </form>
        </div>`;
...
//////////////////////////////////// Menu Icon ////////////////////////////
    ...
    $('.menuicon').on('click', function () {
        ...
        } else
        {
            ...
            /*layout horizonatal not show compact*/
            if ($('.layout:checked').val() == 'horizontal') {
                $('.compact').hide();
            }else{
                $('.compact').show();
            }
...
if(jQuery.isEmptyObject(user_panel_theme_arr) == false){
    //set layout
    if(jQuery.isEmptyObject(user_panel_theme_arr['template_layout']) == false){
        if(user_panel_theme_arr['template_layout'] == 'horizontal'){
            $('.layout').prop("checked", false);
            //$("#layout_horizontal").prop("checked", true);
           $('#layout_horizontal').trigger('click');

           $(".compact").hide();
        }else{
            $('.layout').prop("checked", false);
            //$("#layout_vertical").prop("checked", true);
            $('#layout_vertical').trigger('click');
        }
    }
    //set template color
    if(jQuery.isEmptyObject(user_panel_theme_arr['template_color']) == false){
        $("input.color").val(user_panel_theme_arr['template_color']);
        $('body').css("--primarycolor", $("input.color").val());
        $('.dark').css("--primarycolor", $("input.color").val());
        $('.semi-dark').css("--primarycolor", $("input.color").val());
        $('.semi-dark-alt').css("--primarycolor", $("input.color").val());
    ...
    //set template style
    if(jQuery.isEmptyObject(user_panel_theme_arr['template_style']) == false){
        $('.style').prop("checked", false);
        $("#style_"+user_panel_theme_arr['template_style']).trigger('click');
    }
    //set compact menu
    if(jQuery.isEmptyObject(user_panel_theme_arr['bCompactMenu']) == false){
        if(user_panel_theme_arr['bCompactMenu'] == true || user_panel_theme_arr['bCompactMenu'] == 't' ){
           // $(".sidebar").prop('checked', true);
           $(".sidebar").prop('checked',false);
    ...
    //set small menu
    if(jQuery.isEmptyObject(user_panel_theme_arr['bsmallMenu']) == false){
        if(user_panel_theme_arr['bsmallMenu'] == true || user_panel_theme_arr['bsmallMenu'] == 't' ){
            // $(".menuicon").prop('checked', true);
...
$("#btn_custom_panel").click(function(){
     var form_data = $("#frm_customize_panel").serializeArray();
     form_data.push({name: 'mode', value: 'addCustomizePanelData'});
        $.ajax({
                type: "POST",
