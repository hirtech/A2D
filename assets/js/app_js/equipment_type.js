        ...
        gridtable = $('#datatable-grid').DataTable({  
            "processing": true,
                ...
                "columns": [
                    ...
                    { "data": "vEquipmentType", "sortable":true, "width" : "50%"},
...
function addEditData(id,mode){
    ...
    if(mode == "edit"){
        $("#stmodaltitle").html('Edit Equipment Type');
        $("#st_mode").val('Update');
        $("#equipment_type_id").val(id);
        $("#vEquipmentType").val($("#equipment_type_name_"+id).val());
       
        var status = $("#equipment_type_status_"+id).val();
        if(status == "Active"){
        ...
        $("#stmodaltitle").html('Add Equipment Type');
        $("#st_mode").val('Add');
        $("#iEquipmentTypeId").val('');
        $("#vEquipmentType").val('');
        $("#iStatus").prop('checked',true).change();
    }
    $("#equipment_type_box").trigger('click');
}
...
$("#save_data").click(function(){
    ...
    if(isError == 0){
        ...
        $.ajax({
            type: "POST",
            url: site_url+"master/equipment_type_list",
            data: data_str,
...
function exportExcelSheet(){
    ...
    $.ajax({
        type: "POST",
        url: site_url+"master/equipment_type_list?mode=Excel&iDisplayLength="+iDisplayLength,
        data: $("#frmlist").serializeArray(),
        success: function(data){
            res = JSON.parse(data);
           // console.log(res);
