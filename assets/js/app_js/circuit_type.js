        ...
        gridtable = $('#datatable-grid').DataTable({  
            "processing": true,
                ...
                "columns": [
                    ...
                    { "data": "vCircuitType", "sortable":true, "width" : "50%"},
...
function addEditData(id,mode){
    ...
    if(mode == "edit"){
        $("#stmodaltitle").html('Edit Circuit Type');
        $("#st_mode").val('Update');
        $("#circuit_type_id").val(id);
        $("#vCircuitType").val($("#circuit_type_name_"+id).val());
       
        var status = $("#circuit_type_status_"+id).val();
        if(status == "Active"){
        ...
        $("#stmodaltitle").html('Add Circuit Type');
        $("#st_mode").val('Add');
        $("#iCircuitTypeId").val('');
        $("#vCircuitType").val('');
        $("#iStatus").prop('checked',true).change();
    }
    $("#circuit_type_box").trigger('click');
}
...
$("#save_data").click(function(){
    ...
    if(isError == 0){
        ...
        $.ajax({
            type: "POST",
            url: site_url+"master/circuit_type_list",
            data: data_str,
...
function exportExcelSheet(){
    ...
    $.ajax({
        type: "POST",
        url: site_url+"master/circuit_type_list?mode=Excel&iDisplayLength="+iDisplayLength,
        data: $("#frmlist").serializeArray(),
        success: function(data){
            res = JSON.parse(data);
           // console.log(res);
