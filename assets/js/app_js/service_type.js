        ...
        gridtable = $('#datatable-grid').DataTable({  
            "processing": true,
                ...
                "columns": [
                    ...
                    { "data": "vServiceType", "sortable":true, "width" : "50%"},
...
function addEditData(id,mode){
    ...
    if(mode == "edit"){
        $("#stmodaltitle").html('Edit Service Type');
        $("#st_mode").val('Update');
        $("#service_type_id").val(id);
        $("#vServiceType").val($("#service_type_name_"+id).val());
       
        var status = $("#service_type_status_"+id).val();
        if(status == "Active"){
        ...
        $("#stmodaltitle").html('Add Service Type');
        $("#st_mode").val('Add');
        $("#iServiceTypeId").val('');
        $("#vServiceType").val('');
        $("#iStatus").prop('checked',true).change();
    }
    $("#service_type_box").trigger('click');
}
...
$("#save_data").click(function(){
    ...
    if(isError == 0){
        ...
        $.ajax({
            type: "POST",
            url: site_url+"master/service_type_list",
            data: data_str,
...
function exportExcelSheet(){
    ...
    $.ajax({
        type: "POST",
        url: site_url+"master/service_type_list?mode=Excel&iDisplayLength="+iDisplayLength,
        data: $("#frmlist").serializeArray(),
        success: function(data){
            res = JSON.parse(data);
           // console.log(res);
