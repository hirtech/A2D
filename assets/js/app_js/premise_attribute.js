...
function addEditData(id,mode){
    ...
    if(mode == "edit"){
        $("#stmodaltitle").html('Edit Premise Attribute');
        $("#sattr_mode").val('Update');
        $("#siteattr_id").val($("#sattr_id_"+id).val());
        $("#vAttribute").val($("#sattr_name_"+id).val());
        var status = $("#sattr_status_"+id).val();
        if(status == "Active"){
        ...
        $("#stmodaltitle").html('Add Premise Attribute');
        $("#sattr_mode").val('Add');
        $("#sattr_id").val('');
        $("#vAttribute").val('');
        $("#iStatus").prop('checked',true).change();
...
$("#save_data").click(function(){
    ...
    if(isError == 0){
        ...
        $.ajax({
            type: "POST",
            url: site_url+"master/premise_attribute_list",
            data: data_str,
...
function exportExcelSheet(){
    ...
    $.ajax({
        type: "POST",
        url: site_url+"master/premise_attribute_list?mode=Excel&iDisplayLength="+iDisplayLength,
        data: $("#frmlist").serializeArray(),
        success: function(data){
            res = JSON.parse(data);
           // console.log(res);
