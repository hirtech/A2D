        ...
        gridtable = $('#datatable-grid').DataTable({  
            "processing": true,
                ...
                "columns": [
                    ...
                    { "data": "vEngagement", "sortable":true, "width" : "50%"},
...
function addEditData(id,mode){
    ...
    if(mode == "edit"){
        $("#stmodaltitle").html('Edit Engagement');
        $("#st_mode").val('Update');
        $("#engagement_id").val($("#engagement_id_"+id).val());
        $("#vEngagement").val($("#engagement_name_"+id).val());
       
        var status = $("#engagement_status_"+id).val();
        if(status == "Active"){
        ...
        $("#stmodaltitle").html('Add Engagement');
        $("#st_mode").val('Add');
        $("#iEngagementId").val('');
        $("#vEngagement").val('');
        $("#iStatus").prop('checked',true).change();
    }
    $("#engagement_box").trigger('click');
}
...
$("#save_data").click(function(){
    ...
    if(isError == 0){
        ...
        $.ajax({
            type: "POST",
            url: site_url+"master/engagement_list",
            data: data_str,
...
function exportExcelSheet(){
    ...
    $.ajax({
        type: "POST",
        url: site_url+"master/engagement_list?mode=Excel&iDisplayLength="+iDisplayLength,
        data: $("#frmlist").serializeArray(),
        success: function(data){
            res = JSON.parse(data);
           // console.log(res);
