    ...
    var handleData =function(){
        ...
        gridtable = $('#datatable-grid').DataTable({  
            "processing": true,
                ...
                'bAutoWidth': true,
                "columns": [
                    { "data": "iNetworkId", "sortable":true, "className": "text-center", "width" : "1%"},
                    { "data": "vName", "width" : "50%"},
        ...
        if(access_group_var_add == '1') {
            gridtable.button().add( 0, {
                ...
                action: function ( e, dt, node, config ) {
                     location.href = site_url+"network/network_add";
                }
            });
        }
        //'excel'
...
function exportExcelSheet(){
    ...
    $.ajax({
        type: "POST",
        url: site_url+"network/network_list&mode=Excel&iDisplayLength="+iDisplayLength,
        data: $("#frmlist").serializeArray(),
        success: function(data){
            res = JSON.parse(data);
            isError = res['isError'];
