        ...
        gridtable = $('#datatable-grid').DataTable({  
            "processing": true,
                ...
                "columns": [
                    ...
                    { "data": "vEquipmentManufacturer", "sortable":true, "width" : "50%"},
...
function addEditData(id,mode){
    ...
    if(mode == "edit"){
        $("#stmodaltitle").html('Edit Equipment Manufacturer');
        $("#st_mode").val('Update');
        $("#equipment_manufacturer_id").val(id);
        $("#vEquipmentManufacturer").val($("#equipment_manufacturer_name_"+id).val());
       
        var status = $("#equipment_manufacturer_status_"+id).val();
        if(status == "Active"){
        ...
        $("#stmodaltitle").html('Add Equipment Manufacturer');
        $("#st_mode").val('Add');
        $("#iEquipmentManufacturerId").val('');
        $("#vEquipmentManufacturer").val('');
        $("#iStatus").prop('checked',true).change();
    }
    $("#equipment_manufacturer_box").trigger('click');
}
...
$("#save_data").click(function(){
    ...
    if(isError == 0){
        ...
        $.ajax({
            type: "POST",
            url: site_url+"master/equipment_manufacturer_list",
            data: data_str,
...
function exportExcelSheet(){
    ...
    $.ajax({
        type: "POST",
        url: site_url+"master/equipment_manufacturer_list?mode=Excel&iDisplayLength="+iDisplayLength,
        data: $("#frmlist").serializeArray(),
        success: function(data){
            res = JSON.parse(data);
           // console.log(res);
