...
per_hasModuleAccess("Connection Type", 'List');
$access_group_var_delete = per_hasModuleAccess("Connection Type", 'Delete', 'N');
$access_group_var_status = per_hasModuleAccess("Connection Type", 'Status', 'N');
$access_group_var_add = per_hasModuleAccess("Connection Type", 'Add', 'N');
$access_group_var_edit = per_hasModuleAccess("Connection Type", 'Edit', 'N');
# ----------- Access Rule Condition -----------
# ------------------------------------------------------------
# General Variables
# ------------------------------------------------------------
...
if($mode == "List"){
    ...
    $rs_connection_type = $result_arr['result']['data'];
    $ni = count($rs_connection_type);
    if($ni > 0){
        for($i=0;$i<$ni;$i++){
            $action = '';
            if ($access_group_var_edit == '1') {
                $action .= '<a class="btn btn-outline-secondary" title="Edit" href="javascript:void(0);" onclick="addEditData('.$rs_connection_type[$i]['iConnectionTypeId'].',\'edit\')"><i class="fa fa-edit"></i></a>';
            }
            if ($access_group_var_delete == '1') {
                $action .= '<a class="btn btn-outline-danger" title="Delete" href="javascript:;" onclick="delete_record('.$rs_connection_type[$i]['iConnectionTypeId'].');"><i class="fa fa-trash"></i></a>';
            }
    ...
    $arr_param['iConnectionTypeId'] = $iConnectionTypeId; 
    $arr_param['sessionId']         = $_SESSION["we_api_session_id" . $admin_panel_session_suffix];
    $API_URL = $site_api_url."connection_type_delete.json";
    //echo $API_URL. " ".json_encode($arr_param);exit;
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $API_URL);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
    ...
    $arr_param = array(
        "iConnectionTypeId"         => $_POST['iConnectionTypeId'],
        "vConnectionTypeName"       => $_POST['vConnectionTypeName'],
        "iStatus"                   => ($_POST['iStatus'] !='')?$_POST['iStatus']:"0",
        "sessionId"                 => $_SESSION["we_api_session_id" . $admin_panel_session_suffix]
    );

    $API_URL = $site_api_url."connection_type_edit.json";
    //echo $API_URL." ".json_encode($arr_param);exit;
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $API_URL);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
    ...
    if(isset($result_arr['iConnectionTypeId'])){
        $result['iConnectionTypeId']  = $iConnectionTypeId;
    ...
    $API_URL = $site_api_url."connection_type_add.json";
    //echo $API_URL." ".json_encode($arr_param);exit;
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $API_URL);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
    ...
    if($cnt_export >0) {
        ...
        for($e=0; $e<$cnt_export; $e++) {
            ...
            ->setCellValue('A'.($e+2), $rs_export[$e]['iConnectionTypeId'])
            ->setCellValue('B'.($e+2), $rs_export[$e]['vConnectionTypeName'])
...
$module_name = "Connection Type List";
$module_title = "Connection Type";
